        ...
        while True:
            try:
                try:
                    ...
                    app_log.error("Какие-то ошибки на старте!")
                    continue
                try:
                ...
                try:
                    ...
                    app_log.error("ERROR! Что-то пошло не так в процессе прохождения урока!")
                    sleep(60 * 15)
                    continue
                finally:
                    browser.close()
            except Exception as ex:
                app_log.critical("CRITICAL ERROR! Пинг упал до критического значения!")
                sleep(60 * 60)
                continue
            finally:
                try:
            ...
            if today.hour in (8, 9):
                if today.hour == 9 and today.minute == 0:
                    try:
                        ...
                        text = "Короче не, фраер, скрипт упал ("
                    bot.send_message(YOUR_ID, text)
                    sleep(60 * 60 * 15)
                else:
                    sleep(60)
